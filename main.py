...
from src.interface import HMI, coordinate_file, get_polygon
...
def routine():
    ...
    while not centered and POSTryCnt < POSCounter:
        ...
        json_file = json.load(
            open(
                f"{PROJECT_PATH}/tests/json_stringTesting.json",
                encoding="utf-8",
            )
        )
        ...
        if not centered: 
            try:
                ...
                while time.time() - start < 10:
                    ...
                    print(f"Counter: {POSCounter} (Progress: {0 + POSCounter}/3)")
                    POSCounter += 1
                    
                    if POSCounter <= 3 and (XpulseCalc1 < adjThreshold and YpulseCalc1 < adjThreshold):
                        print("Arrived Early")
                        return True
                    elif POSCounter < 3 and (XpulseCalc1 > adjThreshold or YpulseCalc1 > adjThreshold):
