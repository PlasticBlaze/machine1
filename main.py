...
def recFromArduino():
    if serial1.inWaiting() > 0:
        response = serial1.read().decode("utf-8")


def waitForArduino():
    print(decodestr(serial1.readline()))
    while True:
        ready = decodestr(serial1.readline())
        print(ready)
        if ready == "Comm's Ready":
            break
...
def openCV():
    ...
    # line2 = serial1.readline()
    # print(line2.decode("utf-8"))
    # if line2.decode("utf-8") == "Movement Complete":
