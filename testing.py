...
def coordinateFile(x):
    global polygon
    if x == 1:
        csvX = pd.read_csv('/home/pi/Documents/HCC1-1.csv', usecols=[0], header=0)
        numpX = csvX.to_numpy()
        csvY = pd.read_csv('/home/pi/Documents/HCC1-1.csv', usecols=[1], header=0)
        numpY = csvY.to_numpy()
        lats_long_array = np.column_stack((numpX, numpY))
        return lats_long_array
    elif x == 2:
